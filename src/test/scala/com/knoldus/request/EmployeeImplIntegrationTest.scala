...
class EmployeeImplIntegrationTest extends AnyFunSuite {
  ...
  test("User not created because email is invalid"){
    val Gaurav: Employee = new Employee("Gaurav", "Kumar", 22, 15000, "Intern", "Knoldus", "gaurav.kumar@gmailcom")
    val result = employeeImpl.createEmployee(Gaurav)
    assert(result.isEmpty)
  }
  ...
  test("User can be created"){
    val Gaurav: Employee = new Employee("Gaurav", "Kumar", 22, 15000, "Intern", "Knoldus", "gaurav.km@gmail.com")
    val result = employeeImpl.createEmployee(Gaurav)
